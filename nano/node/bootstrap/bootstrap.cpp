...
{
	auto pull (pull_a);
	if (++pull.attempts < (nano::bootstrap_limits::bootstrap_frontier_retry_limit + (pull.processed / 10000) + (pull.priority * nano::bootstrap_limits::bootstrap_frontier_retry_limit / 2)))
	{
		nano::lock_guard<std::mutex> lock (mutex);
		if (pull.priority)
		{
