...
void nano::active_transactions::request_confirm (std::unique_lock<std::mutex> & lock_a)
{
	...
	for (auto i (roots.get<1> ().begin ()), n (roots.get<1> ().end ()); i != n; ++i)
	{
		...
		else
		{
			if (election_l->announcements > announcement_long)
			{
				...
				if (election_l->local_block && !election_l->ongoing_work_update && election_l->announcements % announcement_long == 1)
				{
					...
					if (difficulty < active_difficulty ())
					{
						...
						node.work_generate (block_l->root (), callback, active_difficulty ());
	...
	// Update average difficulty deque
...
uint64_t nano::active_transactions::active_difficulty ()
{
	...
	uint128_t sum (0);
	for (auto & i : difficulty_deque)
	{
		sum += i;
	}
	if (!difficulty_deque.empty ())
	{
		result = static_cast<uint64_t> (sum / difficulty_deque.size ());
	}
	return result;
}
...
void nano::active_transactions::update_active_difficulty ()
{
	...
	difficulty_deque.push_back (difficulty);
	if (difficulty_deque.size () > announcement_long)
	{
		difficulty_deque.pop_front ();
	}
}
