						...
						if (!error)
						{
							...
							if (work)
							{
								...
								if (!node.store.account_get (transaction, account, info))
								{
									...
									{
										rai::transaction transaction_w (existing->second->store.environment, nullptr, true);
										existing->second->store.work_put (transaction_w, account, work);
									}
									else
									{
								...
								if (!error)
								{
									...
									if (!error && work)
									{
										if (!rai::work_validate (info.head, work))
										{
											...
											rai::transaction transaction_w (existing->second->store.environment, nullptr, true);
											existing->second->store.work_put (transaction_w, source, work);
										}
										else
										{
											error = true;
